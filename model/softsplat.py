...
kernel_Softsplat_updateOutput = '''
	extern "C" __global__ void kernel_Softsplat_updateOutput(
        ...
        //translated;  w11 = (x2 - x) * (y2 - y)
        //w11 = torch.exp(-((x - x1)**2 + (y - y1)**2))
        float fltNorthwest = expf(-(( powf( fltOutputX - (float) (left), 2 ) + powf( fltOutputY - (float) (up), 2))));
        float fltNortheast = expf(-(( powf( fltOutputX - (float) (right), 2 ) + powf( fltOutputY - (float) (up), 2))));
        float fltSouthwest = expf(-(( powf( fltOutputX - (float) (left), 2 ) + powf( fltOutputY - (float) (down), 2))));
        float fltSoutheast = expf(-(( powf( fltOutputX - (float) (right), 2 ) + powf( fltOutputY - (float) (down), 2))));
...
def FunctionSoftsplat(tenInput, tenFlow, tenMetric, strType):
	...
	if strType != 'summation':
		...
		tenOutput = tenOutput[:, :-1, :, :] / tenNormalize + 0.0000001
	# end
